 ...
 * determinism, and #13 says so explicitly.
 *
 * The danger is real but it is not in the freezing — it is in *where* the
 * freezing lives. Freezing the loop would stop the clock, and a run would then
 * depend on how long a frame took, which is exactly the thing the fixed
 * timestep exists to remove. Freezing inside a tick keeps ticks running at
 * sixty a second and keeps the whole simulation a function of its seed and its
 * input log — which is what this file asserts, by running the same log twice
 * through a run that spends most of its time frozen, and once more with one
 * tick of the log changed.
 */
...
function hashRun(sim: GameSim): string {
  ...
  for (let index = 0; index < sim.world.highWater; index++) {
    parts.push(sim.positionX(index), sim.positionY(index), sim.health.data[index * 2] ?? 0);
  }
...
describe('a run with hitstop in it', () => {
  it('freezes often enough for this test to mean something', () => {
  ...
  it('keeps the tick count exactly equal to the number of frames consumed', () => {
    // The fixed timestep is untouched by the freeze: no tick is skipped, none
    // is run twice, and the simulation's clock still tracks the input log.
    const log = scriptedLog();
