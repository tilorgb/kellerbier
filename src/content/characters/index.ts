 ...
 * different **verb**, not a different stat spread. Read the files, not
 ...
 * benched.
 *
 * Der Wolpertinger and D'Sennerin were removed (2026-09) along with the item
 * roster cut — each started with an item the trimmed roster no longer has
 * (Wolpertinger im Rucksack, Kuhschelle), and both are being replaced with
 * hand-picked characters rather than patched to start with something else.
 *
 * Data, like every other roster in `src/content/`: a character is a name, a
 * condition, a stat block and a list of rule ids. Adding the next one is a
 ...
 */
export const AUTHORED_CHARACTERS: readonly CharacterDefinition[] = [alois, resi, koenigLudwig];

/**
 * Who you can actually walk in as right now (#205), in the order the
 * run-start panel lists them.
 *
 * Cut down to Alois alone: the other characters in `AUTHORED_CHARACTERS` were
 * never balanced against each other one at a time (König Ludwig II starting
 * with Ludwigs Schwan *and* 40 Biermarken, next to Alois's neutral baseline,
 * is what surfaced this), so they come back one at a time with their own
 * deliberate balance pass rather than all of them re-appearing at once.
 */
export const CHARACTERS: readonly CharacterDefinition[] = [alois];

export { alois, koenigLudwig, resi };
