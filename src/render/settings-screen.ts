...
export class SettingsScreen implements MenuScreen {
  ...
  /**
   * Rebuilds every tab and row from scratch — used both at construction and
   * from `setLocale`, since every row's label is baked into a `BitmapText`
   * at build time (`app/settings-menu.ts`'s `tabs` are themselves rebuilt
   * first, so this is handed fresh ones already in the new language).
   */
  private rebuildTabs(tabs: readonly SettingsTab[]): void {
    this.tabStrip.removeChildren();
    this.tabStrip.addChild(this.tabUnderline);
    ...
    this.tabIndex = Math.min(this.tabIndex, Math.max(0, this.tabs.length - 1));
    this.focusIndex = this.firstFocusable();
  }

  /** Rebuilds every label in `locale` — call whenever the player changes the language. */
  setLocale(locale: Locale, tabs: readonly SettingsTab[]): void {
    this.locale = locale;
    this.heading.set(t(locale, 'ui.settings.title'));
    this.rebuildTabs(tabs);
    this.refresh();
  ...
  private layOut(): void {
    ...
    this.hint.text = t(this.locale, 'ui.settings.hint');
  ...
  private valueText(row: SettingsRow): string {
    switch (row.kind) {
      ...
      case 'toggle':
        return row.get()
          ? t(this.locale, 'ui.settings.value.on')
          : t(this.locale, 'ui.settings.value.off');
      case 'choice': {
