...
export class StammtischScreen {
  ...
  private layOut(): void {
    ...
    if (this.runOver) {
      ...
      this.addCentred(
        'F: Figur    Links/Rechts: umschaun    C: Lauf kopian    T: zruck zum Lauf',
        centreX,
        hintY,
        UI_PALETTE.textDim,
      );
  ...
  private nextRunRows(state: StammtischView): readonly PanelRow[] {
    ...
    // Seeds are shown in their shareable Crockford spelling (#48), not as the
    // raw 32-bit number — this panel is where a seed gets read off a screen
    // and typed into somebody else's game.
    const daily = state.daily.playedToday;
    rows.push({
      text: state.seedUnlocked ? `Same: ${encodeSeed(this.seed >>> 0)}` : 'Same: no ned dei Sach',
    });
    rows.push({
      text: fit(
