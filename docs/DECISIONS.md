...
## 52. A character is a stat block plus named rules, and a rule is a branch in exactly one system
...
**Save schema v3**, one field: which character the next run starts as. It is a preference, not
progress — a stored id that is unknown or has since re-locked falls back to the first unlocked
character rather than failing, because a `startRun` that throws is a black screen.
